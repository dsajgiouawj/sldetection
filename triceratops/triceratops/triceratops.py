...
class target:
    def __init__(self, ID: int, sectors: np.ndarray,
        ...
        self.cutout_table=cutout_table
        return
    ...
    def remove_star(self, drop_stars: np.ndarray):
        ...
        Args:
            drop_stars (numpy array): Array of (int) TIC IDs
                                      for stars to drop.
        """
        self.pix_coords=np.array(self.pix_coords)
        temp=self.pix_coords[0][~self.stars["ID"].isin(drop_stars)]
        self.pix_coords=np.array([temp])
