...
def gbls_module ():
    ...
    __global__ void geebls(float *blsp, float *lc, float *tu, unsigned int n, unsigned int nf, unsigned int nb, unsigned int kma,  unsigned int kmi, unsigned int kkmi, float fmin, float df){
...
/* Fortran-wise */
/*
        atomicAdd(&cache[j+nb],lc[i+n*blockIdx.y]); 
*/
/* C-wise */

        atomicAdd(&cache[j+nb],lc[i*gridDim.y+blockIdx.y]); 

/* 
    cache[j+nb] <=> y[j], cache[j+nbnbkma] <=> lc[i]
*/
