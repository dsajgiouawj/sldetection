...
if __name__ == "__main__":
    ...
    gap = Conv1D(8,5,activation=act,padding=pad)(ingap)
    gap = Conv1D(8,5,activation=act,padding=pad)(gap)
    gap = MaxPooling1D(7,strides=strmp)(gap)
    gap = Conv1D(16,5,activation=act,padding=pad)(gap)
    gap = Conv1D(16,5,activation=act,padding=pad)(gap)
    gap = MaxPooling1D(7,strides=strmp)(gap)
    gap = Conv1D(32,5,activation=act,padding=pad)(gap)
    gap = Conv1D(32,5,activation=act,padding=pad)(gap)
    gap = MaxPooling1D(7,strides=strmp)(gap)
    gap = Flatten()(gap)

    
    concat = concatenate([wide,local,gap], axis=1)
    ...
    model = Model(inputs=[inwide,inlocal,ingap],outputs=concat)    
