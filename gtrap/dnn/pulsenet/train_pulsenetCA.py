...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', nargs=1, default=["pulsenetCA_train1"],help='', type=str)
    ...
    incib=Input(shape=(np.shape(B)[1],np.shape(B)[2]))
    inast=Input(shape=(np.shape(A)[1],np.shape(A)[2]))
    ...
    cib = Conv1D(8,5,activation=act,padding=pad)(incib)
    cib = Conv1D(8,5,activation=act,padding=pad)(cib)
    cib = MaxPooling1D(7,strides=strmp)(cib)
    cib = Conv1D(16,5,activation=act,padding=pad)(cib)
    cib = Conv1D(16,5,activation=act,padding=pad)(cib)
    cib = MaxPooling1D(7,strides=strmp)(cib)
    cib = Conv1D(32,5,activation=act,padding=pad)(cib)
    cib = Conv1D(32,5,activation=act,padding=pad)(cib)
    cib = MaxPooling1D(7,strides=strmp)(cib)
    cib = Flatten()(cib)

    ast = Conv1D(8,5,activation=act,padding=pad)(inast)
    ast = Conv1D(8,5,activation=act,padding=pad)(ast)
    ast = MaxPooling1D(7,strides=strmp)(ast)
    ast = Conv1D(16,5,activation=act,padding=pad)(ast)
    ast = Conv1D(16,5,activation=act,padding=pad)(ast)
    ast = MaxPooling1D(7,strides=strmp)(ast)
    ast = Conv1D(32,5,activation=act,padding=pad)(ast)
    ast = Conv1D(32,5,activation=act,padding=pad)(ast)
    ast = MaxPooling1D(7,strides=strmp)(ast)
    ast = Flatten()(ast)

    
    concat = concatenate([wide,local,cib,ast], axis=1)
    ...
    model = Model(inputs=[inwide,inlocal,incib, inast],outputs=concat)    
    model.compile(optimizer="adam",loss="binary_crossentropy",metrics=["acc"])
    print(model.summary())
    history=model.fit([Xw,X,B,A],lab,epochs=args.e[0],validation_split=0.5)
    model.save(output)
    print("***********acc************")
